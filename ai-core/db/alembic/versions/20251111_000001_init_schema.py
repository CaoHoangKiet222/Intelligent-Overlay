...
def upgrade() -> None:
    # Extensions - must be created before using vector type
    op.execute("CREATE EXTENSION IF NOT EXISTS vector")
    
    # Enum definitions (no explicit .create calls; let SA/Alembic manage CREATE TYPE)
    ...
    op.create_index("ix_llm_calls_provider_model", "llm_calls", ["provider", "model"])
    ...
    # DESC index via raw SQL for clarity
    op.execute(
        "CREATE INDEX IF NOT EXISTS ix_telemetry_metrics_name_observed_at_desc "
        "ON telemetry_metrics (name, observed_at DESC)"
    )

    # backref FK (after llm_calls exists)
